...
Обновление гайда: **2026-07-16** (балансы по сети, Ping networks, Mission Control, Alchemy private, Discover proxy, Raw multi-wallet).
...
**Правило №3:** **Start = LIVE** (не dry-run): по умолчанию модалка — введите **`LIVE`**. Отключить: Settings → снять «Require type LIVE…» или Apply Sniper preset.  
...
3. Sniper preset / снятие галки в Settings — Start без ввода (power-user).
...
- **Не шарьте** zip с `keys.vault` / своим `config.json` / `auth_cache.bin` / `results`.  
- В zip для друзей — только exe + examples + эта инструкция (см. `README.txt`).
...
## 10. Безопасный zip для друзей (не шарить vault!)

**Никогда** не архивируйте всю папку `Public\` — там могут быть `keys.vault`, `config.json`, `auth_cache.bin`.
...
Создаст `Public\minter-desktop-0.1.0-windows.zip` **только** с allowlist (см. `SHIP_MANIFEST.txt`):
exe + example config/proxies + эта инструкция + README.

---

## 11. Для разработчиков (rebuild)
...
**Итог:**  
zip → папка → `minter-desktop.exe` → Unlock → Wallets → Settings/RPC → Task → **Start** (type **LIVE** by default) → sim → tx → **CONFIRMED** → results / logs.
