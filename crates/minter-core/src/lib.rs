...
mod safe_truncate_tests {
    ...
    fn backs_off_to_char_boundary() {
        // Cyrillic error text (each char is 2 bytes) — a byte cut mid-char must
        // fall back to the previous boundary instead of panicking.
        let s = "ошибка"; // 12 bytes, 6 chars
        let out = safe_truncate(s, 5); // 5 is mid-char → back to 4 bytes = "ош"
